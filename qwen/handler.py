...
This is `runpod-workers/worker-vllm` v2.26.0's own src/handler.py, with ONE addition: every
non-streaming JSON response gets a "where" key added, mirroring where_am_i() in the speech
image's ../handler.py, so a European-mode acta has proof it was actually written inside the EU.
Everything else below is unchanged from the stock handler.
...
When the request body has "stream": true, raw SSE chunks are yielded as they arrive, and no
"where" key is added (a raw SSE chunk cannot carry it without breaking the stream format). Use
stream: false, which is what this project's Qwen provider does anyway (the async /run + /status
pattern, never streaming, because RunPod's serverless OpenAI route has a hard 300s gateway
timeout on streaming requests -- see the "qwen round 2" worklog entry).
"""
...
def where_am_i() -> dict:
    """Which RunPod datacentre this worker is in. Same shape as the speech image's
    where_am_i() in ../handler.py, so both images are read the same way."""
    return {
